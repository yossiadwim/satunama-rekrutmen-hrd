<div class="modal modal-lg fade" id="instrumen-tantangan-berpikir" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Instrumen
                    {{ $analisa_tantangan_berpikir[0]->id_tipe_analisa }}/9
                    {{ Str::title($analisa_tantangan_berpikir[0]->nama_analisa) }}</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <div class="modal-body">

                <div class="accordion" id="tantangan_berpikir">
                    @foreach ($analisa_tantangan_berpikir as $data)
                        <div class="accordion-item">
                            <h2 class="accordion-header">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#tantangan_berpikir_{{ $data->id_jenis_analisa }}"
                                    aria-expanded="false"
                                    aria-controls="tantangan_berpikir_{{ $data->id_jenis_analisa }}">
                                    {{ $data->jenis_analisa_kriteria }}
                                </button>
                            </h2>
                            <div id="tantangan_berpikir_{{ $data->id_jenis_analisa }}"
                                class="accordion-collapse collapse" data-bs-parent="#tantangan_berpikir">
                                <div class="accordion-body">
                                    <form
                                        ...
                                        <label for="range-tantangan_berpikir-{{ $data->id_jenis_analisa }}"
                                            class="form-label">Bobot</label>

                                        <label for=""><span class="badge text-bg-secondary"
                                                id="label_bobot_tantangan_berpikir-{{ $data->id_jenis_analisa }}">0</span></label>

                                        <input type="range" class="form-range form-range-tantangan-berpikir mb-5" min="{{ $data->bobot_minimal }}"
                                            max="{{ $data->bobot_maksimal }}" step="1" value="{{ $data->id_jenis_analisa }}"
                                            id="range-tantangan-berpikir-{{ $data->id_jenis_analisa }}" name="bobot" oninput="getID({{ $data->id_jenis_analisa }})">
