<div class="offcanvas offcanvas-end " data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="notifikasi_admin"
        ...
        <div class="tab-content" id="myTabContent">
            <div class="tab-pane mt-2 fade show active" id="belum_dibaca" role="tabpanel" aria-labelledby="home-tab"
                ...
                @isset($notifikasi_unread)
                    @if ($notifikasi_unread == null)
                        <div class=""></div>
                    @else
                        @for ($i = 0; $i < count($notifikasi_unread); $i++)
                            @if ($notifikasi_unread[$i] != null)
                                @foreach ($notifikasi_unread[$i] as $notif)
                                    <a href="#" class="list-group-item list-group-item-action active"
                                        aria-current="true">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-2 fw-bold ">{!! $notif->data['subject'] !!}</h6>
                                        </div>
                                        <p class="mb-1">{{ $notif->data['nama_pelamar'] }} telah mendaftar pada lowongan
                                            {{ $notif->data['nama_lowongan'] }} </p>
                                    </a>
                                    <p><a href="/markasread-notification-admin/{{ $notif->id }}"
            ...
            <div class="tab-pane fade" id="sudah_dibaca" role="tabpanel" aria-labelledby="profile-tab" tabindex="0">
                @isset($notifikasi_read)
                    @if ($notifikasi_read == null)
                        <div class=""></div>
                    @else
                        @for ($i = 0; $i < count($notifikasi_read); $i++)
                            @if ($notifikasi_read[$i] != null)
                                @foreach ($notifikasi_read[$i] as $notif)
