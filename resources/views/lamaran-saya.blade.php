        ...
        <div class="row mt-4">
            <div class="col-9 ">
                ...
                {{-- <div class="mt-4">
                    ...
                    <table class="table table-borderless " id="table">
                        <tbody>
                            @foreach ($datas[0]->activityLog->sortdesc() as $data_activity)
                                @if ($loop->first)
                                    <tr class="table-active" style="color: blue">
                                ...
                                @endif
                                <tr >
                                    <td> <i class="fa-solid fa-square-full fa-2xs" style="color: #c3c1c1;"></i>
                                        {{ \Carbon\Carbon::parse($data_activity->created_at)->locale('id')->settings(['formatFunction' => 'translatedFormat'])->format('l, j F Y H:i ') }}
                                    </td>
                                    <td>Proses lamaran sudah sampai tahap
                                        {{ $data_activity->status[0]->nama_status }}</td>
                    ...
                    <table class="table table-borderless " id="table-hidden" style="display: none;">
                        <tbody>
                            @foreach ($datas[0]->activityLog->sortdesc()->slice(5) as $data_activity)
                                <tr>
                                    <td> <i class="fa-solid fa-square-full fa-2xs" style="color: #c3c1c1;"></i>
                                        {{ \Carbon\Carbon::parse($data_activity->created_at)->locale('id')->settings(['formatFunction' => 'translatedFormat'])->format('l, j F Y H:i ') }}
                                    </td>
                    ...
                    <a href="#" id="readMore" style="text-decoration: none"><i
                            class="fa-solid fa-chevron-down"></i> Read More</a>
                </div>  --}}
                                ...
                                <div class="tab-content" id="nav-tabContent">
                                    @foreach ($datas as $data)
                                        @foreach ($data->dokumenPelamarLowongan as $dd)
                                            ...
                                            @else
                                                <div class="tab-pane fade" id="tab-pane-seleksi-{{ $dd->id_dokumen }}"
            ...
            <div class="col-3">
                ...
                <div>
                    <p><i class="fa-solid fa-building"></i> {{ $datas[0]->lowongan->departemen->nama_departemen }}</p>
